const url = {
    domains: "https://api.exerra.xyz/v3/phishing/all/domains",
    links: "https://api.exerra.xyz/v3/phishing/all/urls"
}

const domainReq = await fetch(url.domains)
const linksReq = await fetch(url.links)

if (domainReq.status !== 200) throw Error("Exerra Phishing API v3 /all/domains status !== 200")
if (linksReq.status !== 200) throw Error("Exerra Phishing API v3 /all/urls status !== 200")
